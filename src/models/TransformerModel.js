...
try {
  ...
  backendLoaded = true;
  console.log('🚀 WASM backend available for Transformer');
} catch (error) {
  console.log('⚠️ WASM backend not available for Transformer');
}
...
if (!backendLoaded) {
  try {
    ...
    backendLoaded = true;
    console.log('🚀 Node.js backend available for Transformer');
  } catch (error) {
    console.log('⚠️ Node.js backend not available for Transformer');
  }
}
...
if (!backendLoaded) {
  try {
    ...
    console.log('💻 Using CPU backend for Transformer (slower)');
  } catch (error) {
    console.log('❌ No TensorFlow backend available for Transformer!');
  }
}
...
class TransformerModel {
    ...
    async initializeTensorFlow() {
        try {
            console.log('🔧 Initializing TensorFlow backend for Transformer...');
            ...
            for (const backend of backends) {
                try {
                    ...
                    Logger.info(`TensorFlow.js initialized with ${backend} backend for Transformer`, {
                    ...
                    console.log(`⚡ Transformer backend performance test: ${duration}ms`);
                    ...
                    console.log(`❌ ${backend} backend failed for Transformer: ${error.message}`);
                    continue;
                }
            }
            
            if (!success) {
                throw new Error('All TensorFlow backends failed to initialize for Transformer');
            }
            
        } catch (error) {
            Logger.error('Failed to initialize TensorFlow.js for Transformer', { 
                error: error.message 
            });
            throw error;
        }
